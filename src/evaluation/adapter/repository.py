...
class DatasetRepository:
    ...
    async def find_by_id(self, id: str) -> model.EvaluationDataset | None:
        """Find dataset by ID."""
        stmt = sqlalchemy.select(evaluation_schema.EvaluationDatasetSchema).where(
            evaluation_schema.EvaluationDatasetSchema.id == id
        )
...
class RunRepository:
    ...
    async def find_by_id(self, id: str) -> model.EvaluationRun | None:
        """Find run by ID."""
        stmt = sqlalchemy.select(evaluation_schema.EvaluationRunSchema).where(
            evaluation_schema.EvaluationRunSchema.id == id
        )
        result = await self._session.execute(stmt)
        record = result.scalar_one_or_none()
        if record is None:
