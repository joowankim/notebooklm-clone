...
class ListChunksByDocumentHandler:
    ...
    async def handle(self, document_id: str) -> list[ChunkDetail]:
        """List chunks for a document."""
        chunks = await self._repository.list_by_document(document_id)
        return [ChunkDetail.from_entity(chunk) for chunk in chunks]
